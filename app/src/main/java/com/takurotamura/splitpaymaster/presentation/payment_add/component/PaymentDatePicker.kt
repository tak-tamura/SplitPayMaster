...
@Composable
fun PaymentDatePicker(viewModel: PaymentAddScreenViewModel = hiltViewModel()) {
    var showPicker by remember { mutableStateOf(false) }
    val datePickerState = rememberDatePickerState(viewModel.paymentDate)

    val formatter = DateTimeFormatter.ofPattern("uuuu/MM/dd").withZone(ZoneId.systemDefault())
    var textDateState by remember {
        mutableStateOf(formatter.format(Instant.ofEpochMilli(viewModel.paymentDate)))
    }
            ...
            confirmButton = {
                TextButton(
                    onClick = {
                        ...
                        viewModel.paymentDate = datePickerState.selectedDateMillis!!
